...
func NewInventoryProducer(client *nats.Client) *InventoryProducer {
	return &InventoryProducer{
		client: client,
	}
}

func (p *InventoryProducer) PublishProductCreated(ctx context.Context, product model.Product) error {
...
func (p *InventoryProducer) Push(ctx context.Context, product model.Product) error {
...
func (p *InventoryProducer) PublishProductsCreated(ctx context.Context, product []model.Product) error {
	data, err := json.Marshal(product)
	if err != nil {
		return fmt.Errorf("failed to marshal product: %w", err)
	}
